...
class Webhook {
    ...
    */
    sendPayload(resolve, reject, payload, resolveFunction) {
            ...
            snekfetch.post(this.endpoint)
                ...
                .catch((err) => {
                    ...
                    if (e.code === 50006) { // We've sent an empty message
                      process.emitWarning("Discord replied with 50006, can't send empty message")
                      resolve()
                    }

                    if (e.retry_after) { // We are being ratelimited
                        setTimeout(() => {
                            this.sendPayload(resolve, reject, payload, resolveFunction);
                        }, e.retry_after);
                    } else {
                        reject(err);
                    }
    ...
    */
    ...
    send(messageBuilder) {
      return new Promise(((resolve, reject) =>  {
        if (Object.keys(messageBuilder.data.attachments[0]).length > 1 || messageBuilder.data.attachments[0].fields.length > 0 || messageBuilder.data.text) {
          return this.sendPayload(resolve, reject, messageBuilder.data);
        } else {
          reject(new Error("Can't send empty message to Discord"))
        }
      }));
    }
}
